...
app.post('/request-ride', async (req, res) => {
  ...
  try {
    await db.query('INSERT INTO RideRequests (user_id, pickup_location, dropoff_location, pickup_time, child_id, note) VALUES (?, ?, ?, ?, ?, ?)', [userId, pickup_location, dropoff_location || null, pickup_time, child_id, note || null]);
    res.redirect('/dashboard');
  } catch (err) {
